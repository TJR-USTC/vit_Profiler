...
def parse_args():
    ...
    parser.add_argument("--model_names", type=str, default="vit")               # 添加命令行参数，指定模型名称，默认为"vit"
    ...
    parser.add_argument("--dataset", type=str, default="dataset/dataset_split") # 指定数据集名称或路径，默认为 dataset/dataset_split
    parser.add_argument("--batch_size", type=int, default=16)                   # 指定批量大小，默认为16
    parser.add_argument("--epoch", type=int, default=2)                         # 指定训练轮次数，默认为8
    ...
    parser.add_argument("--weight-decay", type=float, default=1e-4)             # 权重衰减（正则化项），默认为 1e-4
...
def setup_data_loaders(args):
    ...
    创建数据加载器
    参数: args - 命令行参数对象
    返回: train_loader, test_loader, val_loader, exp_path
    """
    # 设置数据增强和预处理
    ...
    testset = datasets.ImageFolder(root=os.path.join(args.dataset, 'test'),
                                    transform=transform_test)
    valset = datasets.ImageFolder(root=os.path.join(args.dataset, 'val'),
    ...
    val_loader = DataLoader(valset, batch_size=args.batch_size, num_workers=4,
                            shuffle=False, pin_memory=True)
    ...
    return train_loader, test_loader, val_loader, exp_path
...
def train_one_epoch(model, optimizer, train_loader):
    ...
    for (inputs, targets) in tqdm(train_loader, desc="train"): 
        ...
        if torch.cuda.is_available():  
            # 如果当前设备支持 CUDA 加速，则将输入数据和目标数据送到 GPU 上进行计算，
            # 设置 non_blocking=True 可以使数据异步加载，提高效率。
        ...
        # 记录损失值 
        # 调用 update 方法，传入当前批次的损失值 loss.item() 和该批次的样本数量 inputs.size(0)。 
        # 为了根据样本数量加权计算损失的平均值，确保不同批次的损失贡献相等，而不受批次大小的影响。
...
def train(model, optimizer, train_loader, val_loader, test_loader, scheduler, args, exp_path, tb_writer):
    """
    训练模型
    参数: 
        model - 神经网络模型
    ...
    for epoch in range(args.epoch):
        ...
        tb_writer.add_scalar(tags[3], val_acces, epoch + 1)
    ...
    print(msg_best)
    test_losss, test_acces = evaluation(model, test_loader)
    print("test loss:{:.2f} acc:{:.2f}\n".format(
                test_losss,
                test_acces,
            ))
    msg_test = "test loss:{:.2f} acc:{:.2f}\n".format(
                test_losss,
                test_acces,
            )
    f.write(msg_test)
...
if __name__ == "__main__":
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu_id)  # 设置可见的 GPU 设备
    
    # 创建数据加载器和实验目录
    train_loader, test_loader, val_loader, exp_path = setup_data_loaders(args)
    
    # 创建 TensorBoard 日志目录
    tb_path = "runs/{}/{}/{}".format(args.dataset, args.model_names, args.exp_postfix)
    tb_writer = SummaryWriter(log_dir=tb_path)
    
    # 创建模型
    model = model_dict[args.model_names](num_classes=args.classes_num, pretrained=args.pre_trained)
    if torch.cuda.is_available():
        print(f"CUDA{args.gpu_id} is Avaliable")
        model = model.cuda()
